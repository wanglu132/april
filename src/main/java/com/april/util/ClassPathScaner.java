...
public class ClassPathScaner {
	 ...
	 * logger
	 */
	private final Logger log = LoggerFactory.getLogger(ClassPathScaner.class);
	...
	public static final String SCAN_TYPE_NAME = "Ctrl_shift_T";

	public static final String SCAN_TYPE_HIERARCHY = "Ctrl_T";
	 ...
	 * @param classFilters
	 */
	public ClassPathScaner(Boolean excludeInner, Boolean checkInOrEx, String scanType, String scanName)
		...
		if (scanType.equals(SCAN_TYPE_HIERARCHY)) {
			try {
				superClass = Class.forName(scanName);
	...
	public ClassPathScaner(String scanType, String scanName) throws ClassNotFoundException {
		this(true, true, scanType, scanName);
	}
	...
	private void doScanPackageClassesByJar(String basePackage, URL url, final boolean recursive, Set<Class<?>> classes) {
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				// String classSimpleName = name.substring(name.lastIndexOf('/')
				// + 1);
				// 判定是否符合过滤条件
				String className = name.replace('/', '.');
				if (this.filterClassName(className)) {
	...
	private void doScanPackageClassesByFile(Set<Class<?>> classes, String packageName, String packagePath, boolean recursive) {
		...
		final String fPpackageName = packageName;
		File[] dirfiles = dir.listFiles(new FileFilter() {
			...
			public boolean accept(File file) {
				...
				return filterClassName(fPpackageName + '.' + filename);
			}
		});
	 ...
	 * @return
	 * 
	 *         private boolean filterClassName(String className) { if
	 *         (!className.endsWith(".class")) { return false; } if (null ==
	 *         this.classFilters || this.classFilters.isEmpty()) { return true;
	 *         } String tmpName = className.substring(0, className.length() -
	 *         6); boolean flag = false; for (String str : classFilters) {
	 *         String tmpreg = "^" + str.replace("*", ".*") + "$"; Pattern p =
	 *         Pattern.compile(tmpreg); if (p.matcher(tmpName).find()) { flag =
	 *         true; break; } } return (checkInOrEx && flag) || (!checkInOrEx &&
	 *         !flag); }
	 * @throws ClassNotFoundException
	 */
	private boolean filterClassName(String className) {
		...
		boolean flag = false;
		if (scanType.equals(SCAN_TYPE_NAME)) {
			String tmpName = className.substring(0, className.length() - 6);
			tmpName = tmpName.substring(tmpName.lastIndexOf('.'));
			String tmpreg = "^" + scanName.replace("*", ".*") + "$";
			Pattern p = Pattern.compile(tmpreg);
			if (p.matcher(tmpName).find()) {
				flag = true;
			}
		} else if (scanType.equals(SCAN_TYPE_HIERARCHY)) {
			if (className.endsWith(".class")) {
				try {
					Class<?> cls = Class.forName(className.substring(0, className.length() - 6));
	...
	public static void main(String[] args) throws ClassNotFoundException {
		...
		ClassPathScaner handler = new ClassPathScaner(ClassPathScaner.SCAN_TYPE_HIERARCHY, "com.xcoin.framework.data.cache.Cache");
		Set<Class<?>> calssList = handler.getPackageAllClasses("com.xcoin", true);
